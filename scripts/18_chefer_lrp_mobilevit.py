# -*- coding: utf-8 -*-
"""
First end-to-end run of Chefer-style LRP on MobileViT-S. Visualizes the 
resulting heatmap alongside the input.

Reminder: this is the transformer-stages-only variant. Conv stages and
the unfold boundary are NOT included. See CheferLRPMobileViTMethod's
docstring for the limitation list.
"""
...
n_total_blocks = sum(len(b) for b in method.wrappers_per_block)
print(f"Installed capture: {len(method.wrappers_per_block)} MobileVitBlocks "
      f"with {n_total_blocks} transformer blocks total")
...
axes[1].set_title("Chefer-LRP on MobileViT-S\n(transformer stages only)")
axes[1].axis("off")
...
# --- 5. Save ---
plt.tight_layout()
out_path = FIGURES_DIR / "18_chefer_lrp_mobilevit_dog.png"
